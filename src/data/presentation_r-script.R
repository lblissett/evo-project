...
g20 <-
  ggplot(data = mydata20, aes(x = mydata20$Zyklus, color = Kodierung)) +
  geom_line(aes(y = mydata20$bester.Fitnesswert.Real, col = "Reelle Werte"),
            size = 2) +
  geom_line(aes(y = mydata20$Binaer1P, col = "Binär1P"), size = 2) +
  geom_line(aes(y = mydata20$Binaer2P, col = "Binär2P"), size = 2) +
  scale_x_continuous(limits = limitsX, breaks = stepsX) + scale_y_continuous(limits =
                                                                               limitsY, breaks = stepsY) +
  scale_colour_discrete(name = "Kodierung",
                        breaks = c("Reelle Werte", "Binär1P", "Binär2P"))
...
g20 + theme() + theme(
  ...
  legend.title = element_text(size = 16,
         ...
         y =
           "Fitnesswert",
         title = "Vergleich Kodierungsarten (n = 20, stabile Population)")
...
sg50s <-
  ggplot(data = mydata50s, aes(x = mydata50s$Zyklus, color = Kodierung)) +
  geom_line(
    aes(
      y = mydata50s$bester.Fitnesswert.Real,
      col = "Reelle Werte",
      linetype = "stabil"
    ),
    size = 2
  ...
  geom_line(aes(
    y = mydata50s$Binaer1P,
    col = "Binär1P",
    linetype = "stabil"
  ),
  size = 2) +
  geom_line(aes(
    y = mydata50s$Binaer2P,
    col = "Binär2P",
    linetype = "stabil"
  ),
  size = 2) +
  geom_line(
    aes(
      y = mydata50g$bester.Fitnesswert.Real,
      col = "Reelle Werte",
      linetype = "wachsend"
    ),
    size = 2
  ...
  geom_line(aes(
    y = mydata50g$Binaer1P,
    col = "Binär1P",
    linetype = "wachsend"
  ),
  size = 2) +
  geom_line(aes(
    y = mydata50g$Binaer2P,
    col = "Binär2P",
    linetype = "wachsend"
  ),
  size = 2) +
  ...
  scale_linetype_manual(name = "Population",
                        values = c("stabil" = "twodash", "wachsend" = "solid")) +
...
gn <-
  ggplot(data = mydata5n, aes(x = mydata5n$Zyklus, color = Kodierung)) +
...
bestWorst50 <-
  ggplot(data = myDataBW50, aes(x = myDataBW50$Zyklus, fill = Kodierung)) +
  geom_ribbon(
    aes(
      ymin = myDataBW50$bester.Fitnesswert.Real,
      ymax = myDataBW50$worstReal + 0.5,
      fill = "Reelle Werte"
    )
  ) +
  geom_ribbon(aes(
    ymin = myDataBW50$Binaer1P,
    ymax = myDataBW50$worstBinaer1P,
    fill = "Binaer1P"
  )) +
  geom_ribbon(aes(
    ymin = myDataBW50$Binaer2P,
    ymax = myDataBW50$worstBinaer2P,
    fill = "Binaer2P"
  ))  +
  scale_x_continuous(limits = limitsX, breaks = stepsX) + scale_y_continuous(limits =
                                                                               limitsY, breaks = stepsY) +
